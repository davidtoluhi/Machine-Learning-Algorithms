...
'''
VMLS: Vectorised MultiLayer Sigma-pi .... bad name
...
class vmls(object):
    ...
    raw_labels = []
    error_rate = 1
    weight_range = [0, 900000]

    def __init__(self, data, labels, hidden_layer_nodes_list_rep, learning_rate, iterations, weight_range):
        super(vmls, self).__init__()
        # self.arg = arg

        # a neuron is vector of weights with '-1' as the last element in the vector 
        ...
        if sum(hidden_layer_nodes_list_rep) > 0:
            ...
            # none for input layer and one for output layer
            self.layer_count = len(hidden_layer_nodes_list_rep) + 1
            self.layer_neuron_count = [
                self.input_layer_neuron_count] + hidden_layer_nodes_list_rep + [1]  # output neuron
        else:
            self.layer_neuron_count = [self.input_layer_neuron_count] + [self.input_layer_neuron_count] + [1] #[1] accounts for the output neuron/vector

        for i in range(0, self.layer_count):
            # numpy.random.random(rows, cols)
            # initialisaing matrix of vectors/neurons 
            layer_neurons = numpy.matrix(numpy.random.random(
                (self.layer_neuron_count[i+1], self.layer_neuron_count[i]+1)))
    ...
    def feedForward(self, input_):
        ...
        for z in range(0, self.layer_count):
            ...
            self.layer_outputs.append(
                self.numpySigmoid(inp * self.neurons[z].T))
        pass

    def backpropInput(self, label):
        # because it includes the input layer
        net_activation = self.layer_outputs[self.layer_count][0, 0]
        training_err = label - net_activation
        output_delta = training_err
        self.layer_deltas = []
        self.layer_deltas.append(output_delta)
        output_delta_w = self.learning_rate * output_delta * \
            numpy.c_[self.layer_outputs[self.layer_count-1], 1]
        ...
        for i in range(self.layer_count, 1, -1):
            ...
            self.layer_deltas.insert(0, numpy.multiply(self.numpySigDeriv(
                self.layer_outputs[i-1].T), (self.layer_deltas[0].T * self.neurons[i-1][:, 0:self.layer_neuron_count[i-1]]).T))  # bias

            # print(self.layer_deltas[0])

            delta_w = self.learning_rate * \
                self.layer_deltas[0] * numpy.c_[self.layer_outputs[i-2], 1]
            # print(delta_w)
            # print '**'
            # print(self.neurons[i-1])
        ...
        self.neurons[self.layer_count -
                     1] = self.neurons[self.layer_count-1] + output_delta_w
    ...
    def computeHammingVector(self, hammingLength): 


    def computeSiteAddressCount(self, input_vector_length):
        return 2**input_vector_length
#
